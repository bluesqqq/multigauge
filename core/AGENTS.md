...
This file refines the repository guide for `core/`.
...
Tests live in `core/tests/` and are registered through CTest. Add focused doctest cases and list new files in `core/tests/CMakeLists.txt`. For shared changes, also build the affected ESP32 and/or WebAssembly port as required by the root definition of done.
