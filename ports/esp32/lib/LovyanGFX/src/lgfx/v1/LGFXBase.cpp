...
//----------------------------------------------------------------------------
  ...
  void LGFXBase::drawAnnulus(int32_t x, int32_t y, int32_t r0, int32_t r1) {
    drawCircle(x, y, r0);
    drawCircle(x, y, r1);
  }

  void LGFXBase::fillAnnulus(int32_t x, int32_t y, int32_t r0, int32_t r1) {
    startWrite();

    float inv_r0 = 1.0f / r0;
    float inv_r1 = 1.0f / r1;

    for (int32_t i = -r0; i <= 0; ++i) {
      float yf = i * inv_r0;
      float yf2 = i >= -r1 ? i * inv_r1 : 0.0f;

      float one = 1.0f;
      int32_t widthOuter = int32_t(sqrtf(one - yf * yf) * r0);

      if (i >= -r1) {
        int32_t widthInner = int32_t(sqrtf(one - yf2 * yf2) * r1);
        int32_t width = widthOuter - widthInner;

        if (width > 0) {
          writeFastHLine(x + widthInner, y + i, width);
          writeFastHLine(x + widthInner, y - i, width);
          writeFastHLine(x - widthOuter, y + i, width);
          writeFastHLine(x - widthOuter, y - i, width);
        }
      } else {
        int32_t _x = x - widthOuter;
        writeFastHLine(_x, y - i, widthOuter * 2);
        writeFastHLine(_x, y + i, widthOuter * 2);
      }
    }
