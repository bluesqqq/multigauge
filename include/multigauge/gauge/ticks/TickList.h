...
class TickList : public PropertyObject {
    ...
    private:
        ...
        MG_PROPS_BEGIN()
            MG_PROP(root, "root", "Root Tick", "First tick drawn.", "group")
            MG_PROP(subs, "subs", "Sub Ticks", "Sequentially drawn ticks.", "array")
            MG_PROP(offset, "offset", "Offset", "Value offset to start ticks from.", "number")
        MG_PROPS_END()
