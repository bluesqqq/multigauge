...
#include <multigauge/gauge/GaugeFace.h>     // <-- add
#include <multigauge/editor/Editable.h>     // <-- add (wherever your Editable lives)
...
private:
    ...
    rapidjson::Value exportEditableProps(const Editable& e, rapidjson::Document::AllocatorType& a) const {
        ...
        e.saveProperties(props, a); // your existing reflection hook
        return props;
    }
...
public:
    explicit GaugeEditor(GaugeFace& f) {
        setFace(f);
    }
