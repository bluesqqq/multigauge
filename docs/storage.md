...
`manifest.json` is the installed package manifest generated by the package manager.
...
The package manager generates the internal package id, face ids, and installed file paths from the package and face names when importing. Those generated ids are used for storage and lookup, but they are not part of the package schema itself.
...
1. The package manager generates a unique package id from the package name
2. The package content is written under `packages/<package-id>/`
3. Each face is assigned a unique face id from its name and saved as its own `faces/<face-id>.json` file
4. Embedded assets are extracted into `assets/`
